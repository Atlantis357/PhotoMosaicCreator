...
class KDTree
{
  ...
  private:
     ...
     */
    unsigned Partition(vector<Point<Dim>>& vect, int dimension, unsigned l, unsigned r, unsigned k);

    KDTreeNode* TreeBuilder(vector<Point<Dim>>& vect, int dimension, unsigned l, unsigned r);

    KDTreeNode* CopyTree(KDTreeNode* root);

    Point<Dim> Quickselect(vector<Point<Dim>>& vect, int dimension, unsigned l, unsigned r, unsigned k);

    void Clear(KDTreeNode* root);

    Point<Dim> Find(Point<Dim>& query, KDTreeNode* curr, int dimension) const;

    double SquaredDist(const Point<Dim>& a, const Point<Dim>& b) const;
};
